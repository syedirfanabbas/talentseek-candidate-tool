...
type RecruiterRequest = { id: string; target_role: string; notes?: string; status: string; recruiter_feedback?: string; created_at: string; assigned_recruiter_id?: string | null }
...
export default function RecruiterRequestsPage() {
  ...
  return <main className='min-h-screen bg-slate-50 px-6 py-12'><div className='mx-auto max-w-4xl'>
    ...
    <div className='mt-8 space-y-4'>{loading && <p className='text-slate-500'>Loading requests…</p>}{!loading && requests.length === 0 && <p className='rounded-xl bg-white p-5 text-slate-500 shadow-sm'>No recruiter requests yet.</p>}{requests.map(request => <article key={request.id} className='rounded-2xl bg-white p-6 shadow-sm'><div className='flex flex-wrap items-start justify-between gap-3'><div><h2 className='text-lg font-semibold text-slate-900'>{request.target_role}</h2><p className='mt-1 text-sm text-slate-500'>Submitted {new Date(request.created_at).toLocaleDateString()}</p></div><div className='flex gap-2'>{!request.assigned_recruiter_id && <button onClick={() => void claim(request)} disabled={saving === request.id} className='rounded-lg bg-slate-900 px-3 py-1 text-sm text-white disabled:opacity-50'>Claim request</button>}<select value={request.status} onChange={e => void save(request, { status: e.target.value })} aria-label={`Status for ${request.target_role}`} className='rounded-lg border border-slate-300 px-2 py-1 text-sm text-slate-700'><option value='submitted'>Submitted</option><option value='in_review'>In review</option><option value='feedback_ready'>Feedback ready</option><option value='completed'>Completed</option></select></div></div>{request.notes && <p className='mt-4 rounded-lg bg-slate-50 p-3 text-sm text-slate-700'>{request.notes}</p>}<textarea defaultValue={request.recruiter_feedback || ''} onBlur={e => { if (e.target.value !== (request.recruiter_feedback || '')) void save(request, { recruiter_feedback: e.target.value }) }} placeholder='Add feedback for the candidate…' aria-label={`Feedback for ${request.target_role}`} className='mt-4 min-h-28 w-full rounded-lg border border-slate-300 p-3 text-sm text-slate-900' />{saving === request.id && <p className='mt-2 text-xs text-slate-500'>Saving…</p>}</article>)}</div>
  </div></main>
}
